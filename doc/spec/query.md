...
- フィルタ prefix は `tag:` `-tag:` `feed:` `after:` `before:` の5種のみ。小文字完全一致だけを演算子として解釈する（`Tag:rust` は演算子にならず、ASCII `:` を含む未知 prefix として `INVALID_QUERY`。リテラル検索は `"Tag:rust"` と書く）
...
- 旧記法 `title:` は削除済み。`title:` で始まるトークンのエラーでは、bare term への書き換えを促す専用 hint（`title_filter_removed_use_bare_term`）を返す
- `(` で始まるトークンはブール式グループ（Word Search / Term Groups を参照）
- `-` で始まるトークンは、`-tag:` で始まる場合はタグ否定フィルタ、`-(` で始まる場合は否定グループ、それ以外は否定 term として解釈する（`-title:x` のような形は ASCII `:` を含む未知 prefix として `INVALID_QUERY`）
...
- `feed:` / `after:` / `before:` はそれぞれ 1 回のみ指定可能
- 同じ種類のフィルタトークンを複数回使った場合は `INVALID_QUERY`
...
- 空文字列の term（`""`）は `INVALID_QUERY`。グループ内の quoted リテラル（`(""|foo)`）も同様
...
- `|` `&` `!` が演算子として解釈されるのはグループおよび `tag:` 式の内部のみ。素の term 中ではリテラル（`a|b` は1つの term）
- `(` がグループ開始として解釈されるのはトークン先頭（および `-(`）のみ。`Rust(2024)` は素の term
- グループ内の unquoted リテラルに ASCII `:` を含む場合も Top-Level Rules と同様に `INVALID_QUERY`
- グループには Tag Expression と同じ深さ上限を適用する。リテラル数は Word Search の term 総数上限（32個）に算入して制約する
...
- 括弧の対応が取れた後も、次の空白までは同一トークンとして読む（`(A|B)x` は式の構文エラー）
